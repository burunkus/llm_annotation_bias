...
def access_bias(black_aligned_dataloader, 
    ...
    for batch in white_aligned_dataloader:
        ...
        with torch.no_grad():
            if with_regularization:
                outputs, similarity_score = model(batch)
            else:
                outputs = model(batch)
...
def save_result_to_csv(save_as, results):
    ...
    path = "/home/eokpala/llm_annotation_bias/experiments/assess_bias_binary/"
    # Move a copy to SAVE_PATH
    shutil.copy(save_as + '.csv', SAVE_PATH + save_as + '.csv')
    
    
def main(with_regularization=False):
    ...
    dataset_labels = {
        ...
        "hovy": [None, "Hate"],
        "data": [None, "Hate"] # all data
    }
    
    # Do it all at once since it take 2 minutes on average
    for classifier_name, base_model_config_path in classifiers.items():
        ...
        for fine_tuned_model in fine_tuned_models:
            ...
            result = access_bias(black_aligned_dataloader, 
                                     white_aligned_dataloader, 
                                     tokenizer, 
                                     model, 
                                     num_labels, 
...
if __name__ == "__main__":
    main(with_regularization=WITH_REGULARIZATION)
